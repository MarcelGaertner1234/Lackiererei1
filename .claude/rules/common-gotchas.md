...
## Gotcha 11: Auftragsstart Fallback-Chain (2025-12-17)

```javascript
// FALSCH - abholtermin ist END-Datum!
const startDatum = abholdatum || abholtermin || anliefertermin;

// RICHTIG - anliefertermin ist START-Datum (von saveVerschiebung firstService.datum)
const startDatum = anliefertermin || abholdatum || geplantesStartDatum;
```

**Hintergrund:**
- `saveVerschiebung()` setzt `anliefertermin = firstService.datum` (START)
- `saveVerschiebung()` setzt `abholtermin = lastService.datum` (END)
- Bei mehrtägigen Services ist abholtermin > anliefertermin!

**Bug-Symptom:** Kacheln zeigen END-Datum statt START-Datum
...
| `abholtermin` für Auftragsstart | Falsches Datum (END statt START) |

---

_Aktualisiert: 2025-12-17_

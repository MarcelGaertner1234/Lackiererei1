        ...
        function loadDraft() {
            ...
            if (draft) {
                ...
                if (ageMinutes < 60 && confirm('🔄 Entwurf gefunden! Möchten Sie Ihre letzte Eingabe wiederherstellen?')) {
                    ...
                    console.log('✅ Required fields initialized for restored draft:', restoredServiceTyp);
        ...
        // Entwurf löschen nach erfolgreichem Speichern
        function clearDraft() {
        ...
        // 🔧 2025-11-25: Filter disabled services from UI elements
        function filterDisabledServices(enabledServices) {
            console.log('🔧 Filtere deaktivierte Services:', enabledServices);
            ...
            if (dropdown) {
                Array.from(dropdown.options).forEach(option => {
                    ...
                    if (enabledServices[serviceKey] === false) {
                        ...
                        console.log(`   ❌ Dropdown: ${serviceKey} ausgeblendet`);
                    }
                });
                ...
                if (enabledServices[currentValue] === false) {
                    ...
                    if (firstEnabled) {
                        ...
                        console.log(`   🔄 Dropdown: Wechsel zu ${firstEnabled.value}`);
            ...
            document.querySelectorAll('.checkbox-card[data-service]').forEach(card => {
                ...
                if (enabledServices[serviceKey] === false) {
                    ...
                    console.log(`   ❌ Checkbox: ${serviceKey} ausgeblendet`);
                }
            });

            // Count remaining visible services
            const enabledCount = Object.values(enabledServices).filter(v => v !== false).length;
            console.log(`✅ ${enabledCount}/12 Services aktiv`);
        }

        // ✨ SERVICE-SPEZIFISCHE FELDER TOGGLE
        function toggleServiceFelder() {
            ...
            // Lackierung-Felder auch für "alle" Services zeigen
            if (serviceTyp === 'lackier' || serviceTyp === 'alle') {
                ...
                if (lackierDiv) {
                    ...
                    console.log('🎨 Lackier-Informationen angezeigt');
                }
            }

            // NEU: Bei Dellen → Prüfe Lackschaden-Wert für Lackier-Felder
            ...
            // 🆕 MULTI-SERVICE: Disable checkbox for current primary service
            updateAdditionalServicesCheckboxes(serviceTyp);
        }

        // 🆕 MULTI-SERVICE: Update Additional Services Checkboxes
        function updateAdditionalServicesCheckboxes(primaryService) {
            ...
            console.log(`🆕 Multi-Service: Haupt-Service "${primaryService}" von zusätzlichen Services ausgeschlossen`);
        }

        // 🆕 MULTI-SERVICE: Show/Hide service fields for additional services
        function toggleAdditionalServiceFields() {
            ...
            console.log(`🆕 Multi-Service: ${checkedServices.length} zusätzliche Services ausgewählt:`, checkedServices);
            ...
            checkedServices.forEach(serviceTyp => {
                ...
                if (felderDiv) {
                    felderDiv.style.display = 'block';
                    console.log(`   ✅ Felder angezeigt für: ${serviceTyp}`);
                }
            });
        }

        // 🎨 DELLEN + LACKSCHADEN LOGIC
        function toggleDellenLackierung() {
            ...
            // Nur für Dellen-Service relevant
            if (serviceTyp === 'dellen') {
                if (lackschadenWert === 'ja') {
                    lackierDiv.style.display = 'block';
                    if (lackierungSection) lackierungSection.style.display = 'block'; // NEU: Lackierung-TABELLE einblenden
                    console.log('🎨 Lackier-Felder UND Lackierung-Tabelle angezeigt (Dellen + Lackschaden)');
                    ...
                    if (lackierungSection) lackierungSection.style.display = 'none'; // NEU: Lackierung-TABELLE ausblenden
                    console.log('🔧 Lackier-Felder UND Lackierung-Tabelle versteckt (Dellen ohne Lackschaden)');
        ...
        // ✨ DYNAMISCHE PFLICHTFELDER - Verhindert "invalid form control" Warnungen
        function updateRequiredFields(serviceTyp) {
            // Service-Feld-Mapping: Welche Felder sind für welchen Service PFLICHT?
            ...
            // ALLE möglichen service-spezifischen Felder
            ...
            // SCHRITT 4: Lackart PFLICHT für Lackierung/Alle
            if (serviceTyp === 'lackier' || serviceTyp === 'alle') {
                lackartRadios.forEach(radio => radio.required = true);
            }

            // NEU - SCHRITT 5: Lackschaden bei Dellen → Farbnummer & Lackart PFLICHT
            if (serviceTyp === 'dellen') {
                ...
                if (lackschadenWert === 'ja') {
                    ...
                    lackartRadios.forEach(radio => radio.required = true);
                    console.log('🎨 Farbnummer & Lackart: PFLICHT (Dellen + Lackschaden)');
                }
            }

            console.log(`✅ Required fields updated: ${serviceTyp} (${requiredFields.length} Pflichtfelder)`);
        }

        /**
         * Blendet Tabellen ein/aus basierend auf Service-Typ
         * @param {string} serviceTyp - Der gewählte Service (z.B. "lackier", "dellen", etc.)
         */
        function toggleTabellenByService(serviceTyp) {
            ...
            // Service-Matrix: Welche Tabellen für welchen Service?
            const tabellenConfig = {
                'lackier': { ersatzteile: true, arbeitslohn: true, lackierung: true, materialien: false },
                'reifen': { ersatzteile: true, arbeitslohn: true, lackierung: false, materialien: false },
                'mechanik': { ersatzteile: true, arbeitslohn: true, lackierung: false, materialien: false },
                'pflege': { ersatzteile: false, arbeitslohn: true, lackierung: false, materialien: true }, // Pflegeprodukte
                'tuev': { ersatzteile: false, arbeitslohn: true, lackierung: false, materialien: false },
                'versicherung': { ersatzteile: true, arbeitslohn: true, lackierung: true, materialien: false },
                'glas': { ersatzteile: true, arbeitslohn: true, lackierung: false, materialien: false },
                'klima': { ersatzteile: true, arbeitslohn: true, lackierung: false, materialien: false },
                'dellen': { ersatzteile: false, arbeitslohn: true, lackierung: false, materialien: false }, // Lackierung conditional via toggleDellenLackierung()
                'folierung': { ersatzteile: false, arbeitslohn: true, lackierung: true, materialien: true }, // HYBRID: Lackierung + Materialien
                'steinschutz': { ersatzteile: false, arbeitslohn: true, lackierung: true, materialien: true }, // HYBRID: Lackierung + Materialien
                'werbebeklebung': { ersatzteile: false, arbeitslohn: true, lackierung: true, materialien: true } // HYBRID: Lackierung + Materialien
            };

            // Default: Alle anzeigen (für unbekannte Services oder "alle")
            const config = tabellenConfig[serviceTyp] || { ersatzteile: true, arbeitslohn: true, lackierung: true, materialien: false };
            ...
            console.log(`📊 Tabellen-Sichtbarkeit für Service "${serviceTyp}":`, config);
        }

        // Event Listener für serviceTyp Dropdown (sofort nach Definition)
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (serviceTypSelect) {
                ...
                console.log('✅ serviceTyp Event Listener registered');
            }

            // NEU: Show initial service fields for default selection (Lackierung)
            toggleServiceFelder();
            console.log('✅ Initial service fields toggled on page load');

            // 🆕 MULTI-SERVICE: Event Listeners für Additional Service Checkboxes
            ...
            additionalServiceCheckboxes.forEach(checkbox => {
                window.listenerRegistry.registerDOM(checkbox, 'change', function() {
                    // Re-toggle service fields to show/hide based on checked additional services
                    toggleServiceFelder(); // This already calls updateAdditionalServicesCheckboxes
                    toggleAdditionalServiceFields();
                }, `Additional Service Checkbox: ${checkbox.value}`);
            });
            console.log(`✅ ${additionalServiceCheckboxes.length} Additional Service Checkbox Listeners registered`);

            // NEU: Initiale Tabellen-Sichtbarkeit setzen
            const initialService = document.getElementById('serviceTyp')?.value || 'lackier';
            toggleTabellenByService(initialService);
            console.log('✅ Initiale Tabellen-Sichtbarkeit gesetzt für Service:', initialService);

            // NEU: Event Listener für Lackschaden Select (Dellen + Lackschaden)
            const lackschadenSelect = document.getElementById('lackschaden');
            if (lackschadenSelect) {
                ...
                console.log('✅ lackschaden Event Listener registered');
            }

            // NEU: Event Listener für Folierung Material (Spezialtyp conditional)
            ...
            if (folierungMaterialSelect) {
                ...
                console.log('✅ folierungMaterial Event Listener registered');
            }

            // NEU: Event Listener für Folierung Art (Bereiche conditional)
            ...
            if (folierungArtSelect) {
                ...
                console.log('✅ folierungArt Event Listener registered');
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            // Prüfe ob Nachannahme-Modus (von Partner-Fahrzeugen)
            ...
            if (nachannahmeModus && fahrzeugId) {
                console.log('🔄 Nachannahme-Modus aktiv für Fahrzeug:', fahrzeugId);
                ...
                if (nachannahmeDataStr) {
                    ...
                    console.log('📋 Nachannahme-Daten geladen:', nachannahmeData);
                    ...
                    banner.innerHTML = `
                        ...
                        <div class="info-box-content">
                            <strong>🔄 PHYSISCHE NACHANNAHME</strong>
                            <p>
                                Fahrzeug: <strong>${nachannahmeData.marke} ${nachannahmeData.modell}</strong> • Kennzeichen: <strong>${nachannahmeData.kennzeichen}</strong><br>
                                Die Fahrzeugdaten sind vorausgefüllt. Ergänzen Sie jetzt:<br>
                                📸 Schadenfotos • ✍️ Kundenunterschrift • 📋 Zusätzliche Notizen
                            </p>
                        </div>
                    `;
                    ...
                    // Formular vorausfüllen (read-only)
                    ...
                    // 🔄 NEW: Map serviceData to form fields for ALL 12 services
                    if (nachannahmeData.serviceData && nachannahmeData.serviceTyp) {
                        const sd = nachannahmeData.serviceData;
                        console.log('🔄 Mapping serviceData für Service:', nachannahmeData.serviceTyp, sd);
                        ...
                        switch (nachannahmeData.serviceTyp) {
                            case 'folierung':
                                ...
                                console.log('✅ Folierung-Felder gefüllt');
                                break;
                            ...
                            case 'steinschutz':
                                ...
                                console.log('✅ Steinschutz-Felder gefüllt');
                                break;
                            ...
                            case 'werbebeklebung':
                                ...
                                console.log('✅ Werbebeklebung-Felder gefüllt');
                                break;
                            ...
                            case 'reifen':
                                ...
                                console.log('✅ Reifen-Felder gefüllt');
                                break;
                            ...
                            case 'glas':
                                ...
                                console.log('✅ Glas-Felder gefüllt');
                                break;
                            ...
                            case 'klima':
                                ...
                                console.log('✅ Klima-Felder gefüllt');
                                break;
                            ...
                            case 'dellen':
                                ...
                                console.log('✅ Dellen-Felder gefüllt');
                                break;
                            ...
                            case 'mechanik':
                                ...
                                console.log('✅ Mechanik-Felder gefüllt');
                                break;
                            ...
                            case 'pflege':
                                ...
                                console.log('✅ Pflege-Felder gefüllt');
                                break;
                            ...
                            case 'tuev':
                                ...
                                console.log('✅ TÜV-Felder gefüllt');
                                break;
                            ...
                            case 'versicherung':
                                ...
                                console.log('✅ Versicherung-Felder gefüllt');
                                break;
                            ...
                            case 'lackierung':
                                ...
                                console.log('✅ Lackierung-Felder gefüllt');
                                break;
                        }
                    }
            ...
            try {
                if (typeof initFirebase !== 'undefined') {
                    ...
                    if (window.firebaseInitialized && window.firebaseApp) {
                        ...
                        console.log('✅ Firebase initialisiert für Annahme');
                    }
                }
            } catch (error) {
            ...
            console.log('🔐 Prüfe Auth State für Multi-Tenant...');
            ...
            if (!currentUser) {
                console.warn('⚠️ Kein User eingeloggt - verwende globale Collections (nicht Multi-Tenant)');
                console.warn('   Tipp: Zuerst in index.html einloggen, dann annahme.html öffnen');
                // Optional: Redirect zu Login
                // alert('⚠️ Bitte zuerst einloggen!');
                // safeNavigate('index.html');
                // return;
            } else {
                console.log('✅ Auth State erfolgreich geladen');
                ...
                console.log('   → Verwende Multi-Tenant Collections (z.B. fahrzeuge_' + currentUser.werkstattId + ')');
            }
            ...
            try {
                ...
                if (settings?.profil) {
                    ...
                    console.log('✅ Werkstatt-Branding geladen:', settings.profil.name);
                }

                // 🔧 2025-11-25: Filter disabled services from dropdown and checkboxes
                if (settings?.enabledServices) {
                    filterDisabledServices(settings.enabledServices);
                }
            } catch (error) {
                console.warn('⚠️ Werkstatt-Branding konnte nicht geladen werden:', error);
            }
            ...
            if (!window.istNachannahme) {
                ...
                // (Falls readOnly von früherem Nachannahme-Test noch gesetzt ist)
                const farbnummerInput = document.getElementById('farbnummer');
                if (farbnummerInput) {
                    farbnummerInput.readOnly = false;
                    console.log('✅ Farbnummer: Editierbar (Normaler Workflow)');
                }
            }

            // Form Submit Event Listener registrieren
            console.log('📝 Registriere Form Submit Listener...');
            const formSubmitHandler = async function(e) {
                e.preventDefault();
                console.log('🚀 === ANNAHME SUBMIT GESTARTET ===');

                // ============================================
                // INPUT VALIDATION (Phase 1 Quick Wins)
                // ============================================
                console.log('1️⃣ Validierung...');
                ...
                if (!document.getElementById('marke').value || !document.getElementById('modell').value) {
                    console.log('❌ Validierung fehlgeschlagen: Marke oder Modell fehlt');
                    showToast('Bitte Marke und Modell eingeben!', 'error');
                    return;
                }

                // Validate Farbnummer (NUR für Lackierung & Alle Services)
                const serviceTyp = document.getElementById('serviceTyp').value;
                if (serviceTyp === 'lackier' || serviceTyp === 'alle') {
                    ...
                    if (!document.getElementById('farbnummer').value || !document.getElementById('farbnummer').value.trim()) {
                        showToast('Bitte Farbnummer für Lackierung eingeben!', 'error');
                        document.getElementById('farbnummer').focus();
                        return;
                    }
                    console.log('✅ Farbnummer validiert');
                } else {
                    console.log('⏭️ Farbnummer nicht benötigt für Service: ' + serviceTyp);
                }

                if (photos.length === 0) {
                    console.log('❌ Validierung fehlgeschlagen: Keine Fotos');
                ...
                // Prüfe ob Unterschrift vorhanden
                const signatureData = canvas.toDataURL();
                const emptyCanvas = document.createElement('canvas');
                emptyCanvas.width = canvas.width;
                emptyCanvas.height = canvas.height;
                if (signatureData === emptyCanvas.toDataURL()) {
                    console.log('❌ Validierung fehlgeschlagen: Keine Unterschrift');
                ...
                // ✅ BUG #9 FIX (2025-11-26): werkstattId Validierung
                // KRITISCH: Ohne werkstattId schlagen Duplikat-Queries fehl!
                if (!window.werkstattId) {
                    console.error('❌ KRITISCH: werkstattId nicht gesetzt!');
                    showToast('Fehler: Werkstatt nicht identifiziert. Bitte erneut einloggen.', 'error');
                    return;
                }
                console.log('   ✅ werkstattId:', window.werkstattId);

                console.log('2️⃣ Sammle Formulardaten...');
                ...
                // ═══════════════════════════════════════════════════════════
                // DEBUG: TABELLEN-DATEN VALIDIERUNG
                // ═══════════════════════════════════════════════════════════
                console.log('📊 === TABELLEN-DATEN BEIM SUBMIT ===');
                console.log('   Ersatzteile (global):', ersatzteileData.length, 'Zeilen');
                console.log('   Arbeitslohn (global):', arbeitslohnData.length, 'Zeilen');
                console.log('   Lackierung (global):', lackierungData.length, 'Zeilen');
                console.log('   Materialien (global):', materialienData.length, 'Zeilen');
                console.log('   In annahmeData.pdfImport.editedData:', {
                    ersatzteile: annahmeData.pdfImport?.editedData?.ersatzteile?.length || 0,
                    arbeitslohn: annahmeData.pdfImport?.editedData?.arbeitslohn?.length || 0,
                    lackierung: annahmeData.pdfImport?.editedData?.lackierung?.length || 0,
                    materialien: annahmeData.pdfImport?.editedData?.materialien?.length || 0
                });

                const totalRows = ersatzteileData.length + arbeitslohnData.length + lackierungData.length + materialienData.length;
                if (totalRows > 0) {
                    console.log('✅ Tabellen-Daten vorhanden und bereit für Firestore');
                } else {
                    console.warn('⚠️ WARNUNG: Alle Tabellen sind leer!');
                }

                // Nachannahme-Modus? Dann Duplikat-Prüfung überspringen
                if (window.istNachannahme) {
                    console.log('🔄 Nachannahme-Modus: Überspringe Duplikat-Prüfung');
                } else {
                    // Normale Duplikat-Prüfung
                    console.log('3️⃣ Duplikat-Prüfung...');
                    const kennzeichen = annahmeData.kennzeichen.toUpperCase().replace(/\s+/g, '');
                    let existingVehicles = [];

                    try {
                        if (firebaseInitialized && firebaseApp) {
                            console.log('   → Prüfe Duplikate in Firestore...');
                            existingVehicles = await firebaseApp.getAllFahrzeuge();
                            console.log('   → Gefunden:', existingVehicles.length, 'Fahrzeuge');
                        } else {
                            console.log('   → Prüfe Duplikate in LocalStorage...');
                            existingVehicles = JSON.parse(localStorage.getItem('fahrzeuge') || '[]');
                            console.log('   → Gefunden:', existingVehicles.length, 'Fahrzeuge');
                        }

                        const duplicate = existingVehicles.find(v => {
                            // 🆕 BUGFIX 2025-11-15 (FIX #4): Safe toUpperCase() mit Null-Check
                            const existingKz = (v.kennzeichen || '').toUpperCase().replace(/\s+/g, '');
                            return existingKz === kennzeichen && v.status !== 'abgeschlossen';
                        });

                        if (duplicate) {
                            console.log('⚠️ Duplikat gefunden:', duplicate.kennzeichen);
                            ...
                            let message;
                            if (gleicherKunde) {
                                message = `⚠️ ACHTUNG: Kennzeichen "${annahmeData.kennzeichen}" ist bereits als "${duplicate.status}" vorhanden!\n\n` +
                                          `GLEICHER KUNDE: ${duplicate.kundenname}\n` +
                                          `Fahrzeug: ${duplicate.marke} ${duplicate.modell}\n` +
                                          `Annahme: ${duplicate.datum}\n\n` +
                                          `💡 Möglicherweise ist dieses Fahrzeug bereits in Bearbeitung!\n\n` +
                                          `Trotzdem als NEUEN Auftrag anlegen?`;
                            } else {
                                message = `⚠️ HINWEIS: Kennzeichen "${annahmeData.kennzeichen}" ist bereits vorhanden!\n\n` +
                                          `ABER ANDERER KUNDE:\n` +
                                          `Alter Auftrag: ${duplicate.kundenname} (${duplicate.marke} ${duplicate.modell})\n` +
                                          `Neuer Auftrag: ${annahmeData.kundenname}\n\n` +
                                          `💡 Möglicherweise Fahrzeugwechsel oder verschiedene Besitzer.\n\n` +
                            ...
                            if (!proceed) {
                                console.log('❌ Annahme abgebrochen - Duplikat verhindert');
                                return;
                            }
                            console.log('✅ User hat Duplikat bestätigt - fahre fort');
                        } else {
                            console.log('✅ Kein Duplikat gefunden');
                        }
                    } catch (error) {
                        console.error('⚠️ Duplikat-Prüfung fehlgeschlagen:', error);
                        console.error('   Details:', error.message);
                    }
                }

                // Speicherung
                console.log('4️⃣ Speicherung starten...');
                let materialRequestsCreated = 0; // Track PDF-imported material requests
                try {
                    if (firebaseInitialized && firebaseApp) {
                        console.log('💾 Hybrid-Speicherung: Daten → Firestore, Fotos → LocalStorage');

                        // Nachannahme-Modus: UPDATE statt CREATE
                        if (window.istNachannahme) {
                            console.log('🔄 NACHANNAHME-MODUS: Aktualisiere bestehendes Fahrzeug');
                            ...
                            try {
                                ...
                                console.log('   ✅ Fotos in Firestore gespeichert');
                            } catch (e) {
                                console.error('   ❌ Foto-Speicherung in Firestore fehlgeschlagen:', e);
                            ...
                            showToast('Physische Annahme erfolgreich! Das Fahrzeug wurde mit Fotos und Unterschrift ergänzt.', 'success', 4000);
                        ...
                        // 🔧 BUG #9 FIX (2025-11-20): Email-Format-Validierung BEFORE customer registration
                        if (annahmeData.kundenEmail) {
                            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                            if (!emailRegex.test(annahmeData.kundenEmail)) {
                                showToast('❌ Ungültige Email-Adresse. Bitte korrigieren Sie die Email.', 'error');
                                console.error('❌ Email-Validierung fehlgeschlagen:', annahmeData.kundenEmail);
                                return;
                            }
                        }

                        // 🔧 BUG #6 FIX (2025-11-30): Telefon-Format-Validierung
                        if (annahmeData.telefon) {
                            const digitsOnly = annahmeData.telefon.replace(/\D/g, '');
                            if (digitsOnly.length < 6 || digitsOnly.length > 15) {
                                showToast('❌ Ungültige Telefonnummer. Mindestens 6, maximal 15 Ziffern.', 'error');
                                console.error('❌ Telefon-Validierung fehlgeschlagen:', annahmeData.telefon);
                                return;
                            }
                        }
                        ...
                        if (kundeId) {
                            ...
                            console.log('   ✅ Kunde registriert, ID:', kundeId);
                        } else {
                            console.log('   ⚠️ Keine KundenID erhalten');
                        }
                        ...
                        try {
                            ...
                            console.error('   ❌ Fehler beim Speichern der Fotos in Firestore:', e);
                            console.warn('   → Fallback: Speichere Fotos in LocalStorage');
                        ...
                        // BUGFIX 2025-11-04: Speichere partnerEmail für Firestore Security Rules
                        // Ermöglicht Partner-Zugriff auf ihre eigenen Fahrzeuge
                        if (annahmeData.kundenEmail && annahmeData.kundenEmail.includes('@')) {
                            dataForFirestore.partnerEmail = annahmeData.kundenEmail;
                            console.log('   🔑 partnerEmail gesetzt:', dataForFirestore.partnerEmail);
                        }
                        ...
                        if (!dataForFirestore.pdfImport || !dataForFirestore.pdfImport.editedData) {
                            console.error('❌ KRITISCHER FEHLER: pdfImport.editedData fehlt!');
                            ...
                            console.log('✅ editedData wiederhergestellt:', {
                        ...
                        // Zusätzliche Validierung: Warnen wenn Daten verloren gegangen sind
                        const totalRows = ersatzteileData.length + arbeitslohnData.length + lackierungData.length + materialienData.length;
                        ...
                        if (totalRows > 0 && savedRows === 0) {
                            console.error('⚠️⚠️⚠️ WARNUNG: Tabellen-Daten gehen verloren!');
                            console.error('   Global:', totalRows, 'Zeilen | Zu speichern:', savedRows, 'Zeilen');
                        } else if (totalRows === savedRows) {
                            console.log(`✅ Tabellen-Daten-Validierung erfolgreich: ${totalRows} Zeilen werden gespeichert`);
                        }

                        await firebaseApp.saveFahrzeug(dataForFirestore);
                        console.log('   ✅ Fahrzeug in Firestore gespeichert');

                        // ============================================
                        // ZENTRALE ERSATZTEILE-DB: Dual Save (Auftrag + Collection)
                        // ============================================
                        if (ersatzteileData && ersatzteileData.length > 0) {
                            console.log('   🗄️ ZENTRALE DB: Speichere Ersatzteile in zentrale Collection...');
                            try {
                                ...
                                console.log(`   ✅ ${ersatzteileData.length} Ersatzteile in zentraler DB gespeichert`);
                            } catch (error) {
                                console.error('   ❌ Fehler beim Speichern in zentrale Ersatzteile-DB:', error);
                            }

                            // 🔧 BUG FIX (2025-11-27): Manuell eingegebene Ersatzteile zu Bestellungen konvertieren
                            // PROBLEM: ersatzteileData (manuell) wurde NICHT zu Bestellungen konvertiert
                            // LÖSUNG: autoConvertErsatzteileToBestellungen auch für ersatzteileData aufrufen
                            console.log('   🔄 AUTO-PIPELINE: Konvertiere manuell eingegebene Ersatzteile zu Bestellungen...');
                            try {
                                ...
                                if (bestellungenCreated > 0) {
                                    console.log(`   ✅ ${bestellungenCreated} Bestellungen aus manuellen Ersatzteilen erstellt`);
                                }
                            } catch (error) {
                                console.error('   ❌ Fehler bei Auto-Konvertierung (manuell):', error);
                            }
                        }
                        ...
                        if (pdfData && pdfData.ersatzteile && pdfData.ersatzteile.length > 0) {
                            console.log('   📦 PDF-Import: Speichere Ersatzteile als Material-Requests...');
                            try {
                                ...
                                if (materialRequestsCreated > 0) {
                                    console.log(`   ✅ ${materialRequestsCreated} Ersatzteile als Material-Requests gespeichert`);
                                }
                            } catch (error) {
                                console.error('   ❌ Fehler beim Speichern der Ersatzteile:', error);
                            }

                            // ❌ REMOVED (2025-11-27): pdfData.ersatzteile Konvertierung
                            ...
                            //        Diese alte Konvertierung würde DUPLIKATE erzeugen!
                            // SIEHE: Fix für Bug B - alle Ersatzteile (manuell + PDF) werden jetzt unified behandelt
                        }
                        ...
                        try {
                            ...
                            if (kundenId) {
                                ...
                                console.log('   ✅ Kunde registriert, kundenId:', kundenId);
                            } else {
                                console.warn('   ⚠️ Keine kundenId erhalten');
                            }
                        } catch (error) {
                        ...
                        if (annahmeData.kundenEmail && annahmeData.kundenEmail.includes('@')) {
                            console.log('   → Kunde hat Email:', annahmeData.kundenEmail);

                            try {
                                // STEP 1: Partner Account erstellen/prüfen
                                console.log('   → Calling ensurePartnerAccount...');
                                ...
                                const partnerResult = await ensurePartnerFn({
                                    ...
                                    werkstattId: window.werkstattId // ✅ SECURITY FIX: No 'mosbach' fallback
                                });

                                console.log('   ✅ Partner Account:', partnerResult.data.message);
                                console.log('   → Partner ID:', partnerResult.data.partnerId);
                                console.log('   → Neuer Partner:', partnerResult.data.isNewPartner);

                                // STEP 2: Auto-Login Token erstellen
                                console.log('   → Calling createPartnerAutoLoginToken...');
                                ...
                                console.log('   → Login URL:', tokenResult.data.loginUrl);
                                console.log('   → Gültig bis:', tokenResult.data.expiresAt);

                                // STEP 3: Daten für PDF speichern
                                ...
                                console.log('   🔧 [STEP 2.5] Creating partnerAnfrage for customer portal...');
                                try {
                                    const anfrageId = 'req_' + Date.now();

                                    // 🆕 BUGFIX 2025-11-04: Werkstatt-initiiert workflow
                                    // If price is agreed on-site (vereinbarterPreis > 0), directly mark as "beauftragt"
                                    // Otherwise, normal flow: "neu" → customer confirms → "beauftragt"
                                    ...
                                    const anfrageData = {
                                        ...
                                        // 🆕 BUGFIX 2025-11-04: Add missing paint fields (CRITICAL for paint shop!)
                                        ...
                                        // 🆕 BUGFIX 2025-11-04: Add missing customer & delivery fields
                                        ...
                                        // 🔧 ENTFERNT (2025-11-27): ersatzfahrzeugAdresse war redundant mit abholadresse
                                    ...
                                    console.log('   ✅ [STEP 2.5] partnerAnfrage created:', anfrageId);
                                    console.log('   📊 Data:', {
                                    ...
                                    console.error('   ❌ [STEP 2.5] Failed to create partnerAnfrage:', error);
                                    // Continue anyway - not critical for vehicle intake
                                    // User can still login and create manual requests
                                }

                                console.log('   ✅ QR-Code Daten bereit für PDF');

                                if (annahmeData.isNewPartner) {
                                    console.log('   🆕 NEU-KUNDE → Passwort wird im PDF angezeigt');
                                } else {
                                    console.log('   ✅ BESTANDSKUNDE → Kein Passwort im PDF');
                                }

                            } catch (error) {
                                console.error('   ❌ QR-Code Feature fehlgeschlagen:', error);
                                console.error('   Details:', error.message);
                            ...
                            console.log('   ⏭️ Keine Kunden-Email → QR-Code übersprungen');
                            annahmeData.qrCodeUrl = null;
                            annahmeData.partnerPassword = null;
                        }

                        // 🖼️ FIX: Nov 20, 2025 - Pattern 35: Convert Photo URLs to Base64 for PDF
                        ...
                        if (annahmeData.photos && annahmeData.photos.length > 0) {
                            ...
                            console.log(`   ✅ ${annahmeData.photosBase64.length} Fotos konvertiert`);
                        } else {
                            console.log('   ⏭️ Keine Fotos vorhanden');
                        ...
                        if (pdfBlob && window.storage) {
                            try {
                                console.log('   → Lade PDF zu Firebase Storage hoch...');
                                ...
                                // 🆕 FIX #2: Validate PDF BEFORE upload (Security)
                                try {
                                ...
                                console.log('   ✅ PDF URL in Firestore gespeichert');
                            } catch (uploadError) {
                                console.error('   ⚠️ PDF-Upload fehlgeschlagen:', uploadError);
                                // PDF-Upload ist optional, Fahrzeug ist bereits gespeichert
                            }
                        } else {
                            console.warn('   ⚠️ PDF-Upload übersprungen (pdfBlob oder Storage nicht verfügbar)');
                        }
                    } else {
                        ...
                        try {
                            ...
                            console.log('✅ Fahrzeug in LocalStorage gespeichert');
                            ...
                            if (e.name === 'QuotaExceededError' || e.message.includes('quota')) {
                                showToast('SPEICHER VOLL! Bitte prüfen Sie Ihre Internet-Verbindung. Falls Sie online sind, kontaktieren Sie den Support.', 'error', 6000);
                                return;
                            }
                            throw e;
                        }
                        ...
                        if (pdfBlob && window.storage) {
                            try {
                                ...
                                console.log('   ✅ Firestore mit pdfUrl aktualisiert');
                            } catch (uploadError) {
                                console.error('   ⚠️ PDF-Upload fehlgeschlagen:', uploadError);
                    ...
                    try {
                        ...
                        fahrzeuge.push(annahmeData);
                        localStorage.setItem('fahrzeuge', JSON.stringify(fahrzeuge));
                        console.log('⚠️ Fahrzeug in LocalStorage gespeichert (Firestore-Fehler)');

                        await createPDF(annahmeData);
                        console.log('✅ PDF erstellt (Fallback)');
                    } catch (fallbackError) {
                        console.error('❌ Auch Fallback fehlgeschlagen:', fallbackError);
                        showToast('Speichern fehlgeschlagen! Siehe Console für Details.', 'error');
                        return;
                    }
                }

                // Aufräumen
                console.log('5️⃣ Aufräumen...');
                clearDraft();
                console.log('   ✅ Entwurf gelöscht');

                // Success Message
                const successMessage = document.getElementById('successMessage');
                if (materialRequestsCreated > 0) {
                    successMessage.innerHTML = `✅ Fahrzeug erfolgreich aufgenommen!<br>
                        <small style="font-size: 0.9em; margin-top: 8px; display: block;">
                            📦 ${materialRequestsCreated} Ersatzteil${materialRequestsCreated > 1 ? 'e' : ''} aus PDF importiert
                            → Siehe <a href="material.html" style="color: #4f46e5; text-decoration: underline;">Material-Übersicht</a>
                        </small>`;
                } else {
                    successMessage.innerHTML = '✅ Fahrzeug erfolgreich aufgenommen!';
                }
                successMessage.style.display = 'block';
                console.log('   ✅ Success-Message angezeigt');
        ...
        function toggleQuickMode() {
            ...
            const fieldsToHide = [
                'kostenAufschluesselungSection',  // Kostenaufschlüsselung (beneath notizen)
                'vereinbarterPreisGroup'           // Vereinbarter Preis (beneath kostenAufschluesselung)
            ];

            if (isQuickMode) {
                // Quick Mode ON: Hide sections
                console.log('⚡ Quick Mode aktiviert - zeige nur 3 Felder');
                ...
                showToast('⚡ Quick Mode aktiviert - nur 3 Felder sichtbar', 'info', 3000);

            } else {
                // Quick Mode OFF: Show all sections
                console.log('🔄 Quick Mode deaktiviert - zeige alle Felder');
                ...
                showToast('🔄 Quick Mode deaktiviert - alle Felder sichtbar', 'info', 3000);
            }
        }
         ...
         * Workflow: Meister → Büro vervollständigt später → Kunde bestätigt
         */
        async function saveAsDraft() {
            console.log('📝 === ENTWURF SPEICHERN ===');

            // 🛡️ SECURITY: Only werkstatt users can create drafts (Pattern 6)
            // Partner-Accounts dürfen KEINE Entwürfe erstellen
            if (window.currentUser?.role === 'partner') {
                console.warn('🚫 Partner-Zugriff blockiert! Partner können keine Entwürfe erstellen.');
                showToast('⚠️ Zugriff verweigert. Partner können keine Entwürfe erstellen.', 'error', 5000);
                return;
            }
            ...
            try {
                ...
                if (!kennzeichen || !kundenname || !kundenEmail) {
                    showToast('⚠️ Mindestens Kennzeichen, Kunde und Email erforderlich!', 'error', 5000);
                    return;
                }
                ...
                if (!emailRegex.test(kundenEmail)) {
                    showToast('⚠️ Ungültiges Email-Format!', 'error', 5000);
                    return;
                }

                console.log('✅ Minimum-Validierung erfolgreich');

                // ============================================
                // 2. DATEN SAMMELN (partial data OK für Entwurf)
                // ============================================
                ...
                const entwurfData = {
                    ...
                    // ✅ FIX Bug #5L Phase 5g (2025-11-23): Partner-Daten für Security Rule
                    ...
                    // Fahrzeug-Daten (OPTIONAL für Entwurf)
                    marke: document.getElementById('marke')?.value || '',
                    modell: document.getElementById('modell')?.value || '',
                    baujahrVon: document.getElementById('baujahrVon')?.value || '',
                    kmstand: document.getElementById('kmstand')?.value || '',
                    ...
                    // Service-Daten (OPTIONAL) - ✅ FIX: Nov 17, 2025 - Multi-Service Support (BUG #3)
                    serviceTyp: (() => {
                        ...
                        if (additionalServices.length > 0) {
                            ...
                            console.log(`🆕 Multi-Service Entwurf: ${allServices.length} Services`, allServices);
                            return allServices;  // Array format
                        } else {
                            console.log(`📝 Single-Service Entwurf: ${primaryService}`);
                            return primaryService;  // String format (backwards compatible)
                        }
                    })(),

                    // 🔧 FIX: Nov 17, 2025 - Pattern 33 - Service-Specific Data Collection
                    serviceDetails: getServiceDetails(document.getElementById('serviceTyp')?.value || 'lackier'),

                    // 🔧 FIX: Nov 17, 2025 - Pattern 33 - Additional Services with Details
                    additionalServices: (() => {
                        const primaryService = document.getElementById('serviceTyp')?.value || 'lackier';
                        const additionalServices = Array.from(document.querySelectorAll('.additional-service-checkbox:checked'))
                        ...
                        console.log(`📋 Entwurf Additional Services mit Details:`, additionalServices);
                    ...
                    // 🖼️ Fotos & Unterschrift (FIX: Nov 17, 2025 - Pipeline Data Mapping)
                    photoUrls: photos || [],  // ✅ FIX Bug #7 (2025-11-23): Standardized field name (was: photos)
                    signature: canvas.toDataURL() || null,  // Base64 signature

                    // 🚗 Abholungs-Daten (FIX: 2025-11-27)
                    ...
                    isEntwurf: true,
                    entwurfStatus: 'offen',  // Status: offen → angebot_erstellt → bestaetigt/abgelehnt
                ...
                console.log('📦 Entwurf-Daten:', entwurfData);

                // 🆕 FIX DATA-PIPELINE Bug #1 (Nov 27, 2025): pdfImport Daten hinzufügen
                // CRITICAL: PDF-Daten müssen von Entwurf→KVA→Fahrzeug weitergegeben werden!
                if (pdfData || ersatzteileData.length > 0 || arbeitslohnData.length > 0) {
                    ...
                    // kalkulationData für Rechnungs-Pipeline
                    ...
                    // kostenAufschluesselung für KVA-PDF
                    ...
                    console.log('📋 pdfImport hinzugefügt:', entwurfData.pdfImport);
                    console.log('💰 kostenAufschluesselung:', entwurfData.kostenAufschluesselung);
                }
                ...
                // 🔧 BUG #8 FIX #4 (2025-11-20): Add createdBy audit trail
                const currentUser = firebase.auth().currentUser;

                // 🔧 BUG #4 FIX (2025-11-26): Use getCollection() instead of manual collection name
                await window.getCollection('partnerAnfragen').doc(entwurfId).set({
                    ...entwurfData,
                    // 🆕 AUDIT-TRAIL: User-Tracking für Compliance (Meister der Entwurf erstellt hat)
                    createdBy: currentUser?.displayName || currentUser?.email || 'Meister',
                    createdByUserId: currentUser?.uid || null,
                    createdByEmail: currentUser?.email || null
                });

                console.log(`✅ Entwurf gespeichert in: partnerAnfragen_${window.werkstattId}/${entwurfId}`);

                // ============================================
                // 4. SUCCESS FEEDBACK
                // ============================================

                showToast('✅ Entwurf gespeichert! Sie können ihn später vervollständigen.', 'success', 4000);
                ...
                showToast(`❌ Fehler: ${error.message}`, 'error', 6000);
            }
        }

        /**
         * Als Entwurf speichern + PDF für Kunde erstellen
         * Like saveAsDraft() but also generates QR-Code PDF for customer
         */
        async function saveAsDraftWithPDF() {
            console.log('📄 === ENTWURF SPEICHERN + PDF ERSTELLEN ===');

            // 🛡️ SECURITY: Only werkstatt users can create drafts
            if (window.currentUser?.role === 'partner') {
                console.warn('🚫 Partner-Zugriff blockiert!');
                showToast('⚠️ Zugriff verweigert. Partner können keine Entwürfe erstellen.', 'error', 5000);
                return;
            }
            ...
            try {
                ...
                if (!kennzeichen || !kundenEmail) {
                    showToast('⚠️ Mindestens Kennzeichen und Email erforderlich!', 'error', 5000);
                    return;
                }

                // kundenname nur im Normalen Mode required
                if (!isQuickMode && !kundenname) {
                    showToast('⚠️ Kundenname erforderlich im Normalen Modus!', 'error', 5000);
                    return;
                }
                ...
                const entwurfData = {
                    ...
                    // Service-Daten - ✅ FIX: Nov 17, 2025 - Multi-Service Support (BUG #3)
                    serviceTyp: (() => {
                        const primaryService = document.getElementById('serviceTyp')?.value || 'lackier';
                        const additionalServices = Array.from(document.querySelectorAll('.additional-service-checkbox:checked'))
                            .map(cb => cb.value)
                        ...
                        if (additionalServices.length > 0) {
                            const allServices = [primaryService, ...additionalServices];
                            console.log(`🆕 Multi-Service Entwurf (with PDF): ${allServices.length} Services`, allServices);
                            return allServices;  // Array format
                        } else {
                            console.log(`📝 Single-Service Entwurf (with PDF): ${primaryService}`);
                            return primaryService;  // String format (backwards compatible)
                        }
                    })(),
                    ...
                    additionalServices: (() => {
                        ...
                        console.log(`📋 Entwurf (with PDF) Additional Services mit Details:`, additionalServices);
                        return additionalServices.length > 0 ? additionalServices : null;
                    })(),
                ...
                console.log('🔑 Erstelle Auto-Login Token für Partner-Login...');
                showToast('⏳ Erstelle Partner-Login...', 'info', 2000);
                ...
                console.log('✅ Auto-Login Token erstellt');
                ...
                console.log('📄 Generiere PDF mit QR-Code...');
                showToast('📄 Erstelle PDF...', 'info', 2000);

                // FIX: Nov 21, 2025 - Pass full entwurfData instead of only 4 params
                await erstelleEntwurfPDF(entwurfData, qrCodeUrl);

                // ============================================
                // 6. SUCCESS FEEDBACK
                // ============================================

                showToast('✅ Entwurf gespeichert & PDF heruntergeladen!', 'success', 4000);
        ...
        async function loadKunden() {
            try {
                ...
                console.log(`✅ ${allKunden.length} Kunden geladen`);
        ...
        function updateKundenDropdown() {
            ...
            dropdown.innerHTML = '<option value="">🆕 Neuer Kunde</option>';
        ...
        function selectKunde() {
            ...
            if (kunde) {
                ...
                // 🆕 BUGFIX 2025-11-04: Email automatisch ausfüllen
                if (kunde.email) {
                ...
                // 🆕 BUGFIX 2025-11-04: Telefon automatisch ausfüllen
                if (kunde.telefon) {
                ...
                console.log(`✅ Stammkunde ausgewählt: ${kunde.name} (${kunde.anzahlBesuche || 0} Besuche)`);
                if (kunde.email) console.log(`   📧 Email: ${kunde.email}`);
                if (kunde.telefon) console.log(`   📞 Telefon: ${kunde.telefon}`);
            }
        }
        ...
        // Radio-Button visuelle Rückmeldung
        ...
        const photoInputHandler = function(e) {
            ...
            for (const file of files) {
                if (file.size > 5 * 1024 * 1024) {
                    showToast(`Foto "${file.name}" ist zu groß (${(file.size / 1024 / 1024).toFixed(1)} MB)! Bitte kleineres Foto verwenden (max. 5 MB).`, 'warning', 5000);
                    e.target.value = '';
                    return;
                }
            }

            // ✅ MIGRATED TO FIREBASE STORAGE (Phase: Nov 17, 2025)
            // Old: Base64 compression → Firestore (1MB limit issues)
            // New: Direct upload to Firebase Storage → Store URLs
            // ✅ SECURITY FIX: No 'mosbach' fallback - werkstattId must be validated at login
            if (!window.werkstattId) {
            ...
            (async () => {
                for (const file of files) {
                    try {
                        ...
                        console.log(`✅ Foto hochgeladen: ${storagePath}`);
        ...
        async function removePhoto(index) {
            ...
            if (photoUrl && photoUrl.startsWith('https://firebasestorage.googleapis.com')) {
                try {
                    ...
                    console.log(`✅ Foto aus Storage gelöscht: ${photoRef.fullPath}`);
                } catch (error) {
                    // Ignore errors (file might already be deleted or not exist)
                    console.warn('⚠️ Konnte Foto nicht aus Storage löschen:', error.message);
                }
            }
        ...
        window.addEventListener('themeChanged', () => {
            initCanvas();
            console.log('✅ Canvas colors updated for theme:', document.documentElement.getAttribute('data-theme'));
        });

        // Formular-Daten sammeln
        // ✨ SERVICE-DETAILS SAMMELN
        function getServiceDetails(serviceTyp) {
            ...
            switch(serviceTyp) {
                ...
                case 'lackier':
                    ...
                    // 🔧 FIX (2025-11-27): Ersatzfahrzeug-Daten für Primary Lackier-Service
                ...
                // 🆕 FIX (2025-11-27): case 'lackierung' für Primary Lackierung-Service
                // War vorher NICHT vorhanden - nur 'lackier' für Secondary Services
                case 'lackierung':
                    ...
                    // Ersatzfahrzeug-Daten IMMER hinzufügen
        ...
        async function handlePdfUpload(event) {
            ...
            if (!file || file.type !== 'application/pdf') {
                showToast('❌ Bitte wählen Sie eine PDF-Datei', 'error');
                return;
            }
            ...
            try {
                ...
                // OpenAI GPT-4 Vision ist PRIMARY für 100% Genauigkeit
                // Regex nur als Fallback wenn OpenAI fehlschlägt
                // =====================================================================
                ...
                try {
                    ...
                    console.log('🤖 [PRIMARY] Trying OpenAI GPT-4 Vision...');
                    pdfExtraction = await parseWithOpenAI(file);
                    console.log('✅ [PRIMARY] OpenAI parsing successful');
                } catch (openaiError) {
                    // FALLBACK: Regex-based parsing (only if OpenAI fails)
                    console.warn('⚠️ [FALLBACK] OpenAI parsing failed, falling back to regex:', openaiError);
                    showToast('⚠️ AI-Parsing fehlgeschlagen, verwende Regex-Fallback...', 'warning', 3000);

                    pdfExtraction = await parseDatPdf(file);
                    pdfExtraction.source = 'pdf-regex';
                    console.log('✅ [FALLBACK] Regex parsing completed');
                }
                ...
                if (pdfData && pdfData.source === 'text' && pdfData.ersatzteile && pdfData.ersatzteile.length > 0) {
                    const confirmed = confirm(
                        `💡 Es wurden bereits ${pdfData.ersatzteile.length} Ersatzteile aus Text extrahiert.\n\n` +
                        `Möchten Sie diese mit PDF-Daten KOMBINIEREN für höhere Zuverlässigkeit?\n\n` +
                        `✅ JA → Beide Methoden kombinieren (empfohlen! 97-98% Zuverlässigkeit)\n` +
                        `❌ NEIN → Nur PDF-Daten verwenden`
                    );

                    if (confirmed) {
                        // Merge: PDF is primary (higher accuracy from column detection)
                        console.log('🔄 User wählte MERGE → Kombiniere PDF (primary) + Text (secondary)');
                        ...
                        showToast(`✅ Kombiniert: ${pdfData.ersatzteile.length} Ersatzteile (PDF + Text)`, 'success');
                    } else {
                        // Replace text with PDF
                        console.log('⚠️ User wählte REPLACE → Text-Daten werden überschrieben');
                        ...
                        renderLackierungPreview(pdfData.lackierung);
                        updateGesamtsumme(pdfData);
                        showToast('✅ Fahrzeugdaten aus PDF übernommen!', 'success');
                    }
                } else {
        ...
        async function convertPdfToImage(pdfFile) {
            console.log('🖼️ [PDF→PNG] Converting PDF to PNG images for OpenAI Vision API...');
            ...
            try {
                ...
                console.log(`📄 [PDF→PNG] PDF loaded: ${pdf.numPages} page(s)`);

                const pngImages = [];

                // Convert ALL pages to PNG
                for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                    console.log(`🎨 [PDF→PNG] Processing page ${pageNum}/${pdf.numPages}...`);
                    ...
                    console.log(`   ✅ Page ${pageNum}: ${sizeKB} KB PNG`);

                    pngImages.push(pngBase64);
                }

                const totalSizeKB = (pngImages.reduce((sum, img) => sum + img.length, 0) / 1024).toFixed(2);
                console.log(`✅ [PDF→PNG] All ${pdf.numPages} pages converted: ${totalSizeKB} KB total`);

                return pngImages;

            } catch (error) {
                console.error('❌ [PDF→PNG] Conversion failed:', error);
        ...
        async function parseWithOpenAI(pdfFile) {
            console.log('🤖 [OPENAI] Starte PDF-Parsing mit OpenAI GPT-4 Vision...');

            showToast('⏳ PDF wird analysiert mit AI...', 'info', 3000);

            try {
                // Convert PDF to PNG images (ALL pages - OpenAI Vision API only accepts images, not PDFs)
                console.log('📄 [OPENAI] Converting PDF to PNG images...');
                ...
                console.log(`✅ [OPENAI] ${pngImagesArray.length} PNG images ready: ${totalSizeKB} KB total`);

                // Call Cloud Function (Region bereits in firebase-config.js auf europe-west3 gesetzt!)
                console.log('☁️ [OPENAI] Calling Cloud Function parseDATPDF (europe-west3)...');
                ...
                console.log('✅ [OPENAI] Parsing erfolgreich:', extractedData);
                ...
                showToast('✅ PDF erfolgreich analysiert mit AI!', 'success');
        ...
        async function parseDatPdf(file) {
            ...
            for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                ...
                // Nutze x/y-Koordinaten statt flattened text für 95% Erfolgsrate
                // =====================================================================
                ...
                // =====================================================================
                if (window.DEBUG) console.log(`🔍 [COLUMN-DEBUG] Page ${pageNum}: ${tableRows.length} rows found`);
                tableRows.forEach((row, i) => {
                    ...
                    row.columns.forEach((col, colIdx) => {
                        if (/^\d{10}$/.test(col)) {
                            console.log(`      ✓ Column ${colIdx} matches 10-digit ETN: "${col}"`);
                        } else if (/^\d{4,8}[A-Z]\d{0,2}$/.test(col)) {
                            console.log(`      ⚠️ Column ${colIdx} matches 6-8 digit ETN: "${col}" (NOT YET SUPPORTED)`);
                        }
                    });
                });
                console.log('---');

                // Fahrzeugdaten: Nutze alte Regex-Methode (funktioniert gut für Header-Daten)
                ...
                tableRows.forEach(row => {
                    ...
                    if (row.columns.length >= 5) {
                        ...
                        if (etnIndex >= 0 && etnIndex < row.columns.length - 2) {
                            console.log(`   🔍 [STAGE2] Found ETN at column ${etnIndex} (${etnPattern}): "${row.columns[etnIndex]}"`);

                            const etn = row.columns[etnIndex];
                            const benennung = row.columns[etnIndex + 1];
                            const anzahl = parseInt(row.columns[etnIndex + 2]) || 1;

                            // Preise können in den letzten 2 Spalten sein
                            let einzelpreis = 0;
                            let gesamtpreis = 0;
                            ...
                            if (etn && benennung && benennung.length > 2) {
                                ...
                                console.log(`✅ [COLUMN] Ersatzteil ${extractedData.ersatzteile.length}:`, {
                                    etn,
                ...
                // Fallback-Chain: Column → Row-based → Flattened text
                ...
                if (ersatzteilCountAfterColumn === 0) {
                    console.log('⚠️ Column-based extraction fand nichts, versuche Row-basiertes Regex...');
                ...
                if (ersatzteilCountAfterRows === 0) {
                    console.log('⚠️ Row-based Regex fand nichts, fallback zu flattened text...');
        ...
        function extractFahrzeugdaten(text, extractedData) {
            ...
            const herstellerMatch = text.match(/Hersteller:?\s*([A-ZÄÖÜa-zäöüß]+)/i);
            if (herstellerMatch && !extractedData.fahrzeugdaten.marke) {
                extractedData.fahrzeugdaten.marke = herstellerMatch[1];
                console.log('✅ Marke gefunden:', herstellerMatch[1]);
            }
            ...
            if (haupttypMatch && !extractedData.fahrzeugdaten.modell) {
                ...
                console.log('✅ Modell gefunden:', extractedData.fahrzeugdaten.modell);
            }
            ...
            if (vinMatch && !extractedData.fahrzeugdaten.vin) {
                ...
                console.log('✅ VIN gefunden:', vinMatch[1]);
            }
            ...
            const kennzeichenPatterns = [
                /(?:Kontrollschild|Kennzeichen|amtl\.\s+Kennzeichen)[:\s]+([A-ZÄÖÜ]{1,3}[-\s][A-ZÄÖÜ]{1,2}\s?\d{1,4}[A-Z]?)/i,
                /([A-ZÄÖÜ]{1,3}[-\s][A-ZÄÖÜ]{1,2}\s?\d{1,4}[A-Z]?)\s+(?:Kennzeichen|Kontrollschild)/i
            ];
            ...
            for (const pattern of kennzeichenPatterns) {
                ...
                if (match && !extractedData.fahrzeugdaten.kennzeichen) {
                    ...
                    console.log('✅ Kennzeichen gefunden:', extractedData.fahrzeugdaten.kennzeichen);
                    break;
                }
            }
            ...
            if (farbcodeMatch && !extractedData.fahrzeugdaten.farbcode) {
                ...
                console.log('✅ Farbcode gefunden:', farbcodeMatch[1]);
            }

            // Fahrzeugfarbe: "LACKIERUNG PLATINIUM-GRAU/TYP" oder "Farbe: Grau"
            const farbMatch = text.match(/(?:LACKIERUNG|Farbe)[:\s]+([A-ZÄÖÜ\-\s]+?)(?:\s+\/|\s+TYP|$)/i);
            if (farbMatch && !extractedData.fahrzeugdaten.farbname) {
                extractedData.fahrzeugdaten.farbname = farbMatch[1].trim().replace(/-/g, ' ');
                console.log('✅ Farbname gefunden:', extractedData.fahrzeugdaten.farbname);
            }
        }

        function extractErsatzteile(text, extractedData) {
            if (window.DEBUG) console.log('🔍 Extracting Ersatzteile from PDF...');

            // =====================================================================
            // STAGE 3: Verbessertes Regex Pattern für zuverlässige Preis-Extraktion
            // STAGE 4: Unterstützung für optionalen RC/DVN Prefix (z.B. "E 45612")
            // =====================================================================
            // DAT-Tabelle Regex (ETN | Benennung | Anzahl | Preis/Stück | Gesamtpreis)
            ...
            // Benennung: GREEDY (nicht non-greedy!) - matched alle Wörter bis zur Anzahl
            //   - Non-greedy *? stoppte zu früh bei "SCHEINWERFER R." → matched nur "SCHEINWERFER"
            //   - Greedy * matched alle Wörter inkl. "R." → "SCHEINWERFER R." ✓
            // (?:[A-Z]\s+\d+\s+)? = Optional: RC Code + DVN vor ETN (z.B. "E 45612")
            const ersatzteilRegex = /(?:[A-Z]\s+\d+\s+)?(\d{10}|\d{4,8}[A-Z]\d{0,2})\s+([A-ZÄÖÜa-zäöüß0-9\.\,\-\(\)\/]+(?:\s+[A-ZÄÖÜa-zäöüß0-9\.\,\-\(\)\/]+)*)\s+(\d+)\s+([\d\'\.\,]+)\s+([\d\'\.\,]+)/g;
            ...
            while ((match = ersatzteilRegex.exec(text)) !== null) {
                ...
                if (etn && benennung && anzahl > 0 && einzelpreis > 0) {
                    ...
                    count++;
                    console.log(`✅ Ersatzteil ${count} gefunden:`, {
                        etn,
                        benennung,
                        anzahl,
                        einzelpreis: einzelpreis.toFixed(2),
            ...
            if (count === 0) {
                console.warn('⚠️ Keine Ersatzteile gefunden. Überprüfe PDF-Format.');
            } else {
                console.log(`✅ Insgesamt ${count} Ersatzteile extrahiert`);
            }
        }
        ...
        // Verhindert dass greedy pattern über mehrere Zeilen matcht
        // =====================================================================
        function extractErsatzteileFromRows(tableRows, extractedData) {
            if (window.DEBUG) console.log('🔍 [ROW-BASED] Extracting Ersatzteile from table rows...');

            // Gleiches Regex wie extractErsatzteile() - aber angewendet auf einzelne Rows
            const ersatzteilRegex = /(?:[A-Z]\s+\d+\s+)?(\d{10}|\d{4,8}[A-Z]\d{0,2})\s+([A-ZÄÖÜa-zäöüß0-9\.\,\-\(\)\/]+(?:\s+[A-ZÄÖÜa-zäöüß0-9\.\,\-\(\)\/]+)*)\s+(\d+)\s+([\d\'\.\,]+)\s+([\d\'\.\,]+)/;

            let count = 0;
            tableRows.forEach((row, rowIndex) => {
                // Join columns zu einem String für diese Row
                ...
                if (match) {
                    ...
                    if (etn && benennung && anzahl > 0 && einzelpreis > 0) {
                        ...
                        count++;
                        console.log(`✅ [ROW ${rowIndex}] Ersatzteil ${count}:`, {
                            etn,
            ...
            if (count === 0) {
                console.warn('⚠️ [ROW-BASED] Keine Ersatzteile gefunden');
            } else {
                console.log(`✅ [ROW-BASED] Insgesamt ${count} Ersatzteile extrahiert`);
            }
        }
        ...
        // ChatGPT-Style Extraction mit x/y-Koordinaten für 95% Erfolgsrate
        // ============================================================================

        /**
         * Extrahiert Tabellen-Zeilen aus PDF.js textContent items
         * Nutzt x/y-Koordinaten für Spalten-Erkennung (wie ChatGPT)
        ...
        function extractTableRows(items) {
            ...
            for (const [y, items] of rowMap.entries()) {
                ...
                // Erkenne Spalten-Breaks (Lücke > 20px zwischen Items)
                const columns = [];
                let currentColumn = '';
                let lastX = -1;
         ...
         * Beispiel: "1'532.10" oder "1.532,10" → 1532.10
        ...
        function parseGermanPrice(priceStr) {
            ...
            const cleaned = priceStr
                ...
                .replace(',', '.');       // Komma → Punkt (Dezimaltrennzeichen)
        ...
        // Kombiniert PDF + Text Extraktionen intelligent für 97-98% Zuverlässigkeit
        // ============================================================================
        ...
        function mergeExtractions(primary, secondary, strategy = 'hybrid') {
            console.log(`🔄 [MERGE] Starting merge - Strategy: ${strategy}`);
            console.log(`📊 Primary: ${primary.ersatzteile.length} parts (source: ${primary.source || 'unknown'})`);
            console.log(`📊 Secondary: ${secondary.ersatzteile.length} parts (source: ${secondary.source || 'unknown'})`);
            ...
            primary.ersatzteile.forEach(primaryPart => {
                ...
                if (secondaryPart) {
                    ...
                    if (priceDiffPercent > 5) {
                        ...
                        console.warn(`⚠️ [MERGE] Preiskonflikt bei ${primaryPart.etn}: Primary=${primaryPart.einzelpreis.toFixed(2)}€, Secondary=${secondaryPart.einzelpreis.toFixed(2)}€ (${priceDiffPercent.toFixed(1)}%)`);
                    }
            ...
            secondaryMap.forEach(secondaryPart => {
                ...
                console.log(`➕ [MERGE] Added from secondary: ${secondaryPart.etn} - ${secondaryPart.benennung.substring(0, 30)}...`);
            });

            // Summary logging
            console.log(`✅ [MERGE] Complete! Results:`);
            ...
            if (merged.mergeStats.priceConflicts.length > 0) {
                showToast(
                    `⚠️ ${merged.mergeStats.priceConflicts.length} Preiskonflikt${merged.mergeStats.priceConflicts.length > 1 ? 'e' : ''} gefunden (siehe Console)`,
                    'warning',
                    5000
                );
            }
        ...
        function fillFormFromPdf(pdfData) {
            ...
            if (fd.marke) {
                ...
                // Prüfe ob Marke im Dropdown existiert
         ...
         * Ermöglicht spätere Statistiken, API-Integrationen und Ersatzteil-Suche
        ...
        async function saveErsatzteileToCentralDB(ersatzteile, fahrzeugId, kennzeichen, kundenname) {
            ...
            // ✅ SECURITY FIX: No 'mosbach' fallback - werkstattId must be validated at login
            if (!window.werkstattId) {
                console.error('❌ werkstattId nicht gesetzt - Ersatzteile werden nicht gespeichert');
                return 0;
            }
         ...
         * @param {string} kennzeichen - Kennzeichen des Fahrzeugs für Zuordnung
        ...
        async function savePdfErsatzteileAsRequests(ersatzteile, kennzeichen) {
            ...
            console.log(`📦 Speichere ${ersatzteile.length} Ersatzteile als Material-Requests...`);
            ...
            for (const teil of ersatzteile) {
                try {
                    ...
                    const requestData = {
                        ...
                        description: `ETN: ${teil.etn} - ${teil.benennung} (Anzahl: ${teil.anzahl}, Einzelpreis: ${teil.einzelpreis.toFixed(2)} €)`,
                        requestedBy: userName,
                        timestamp: new Date().toISOString(),
                        status: 'pending',
                        // Zusätzliche Daten für PDF-Import
                        source: 'pdf-import',
                    ...
                    console.error('❌ Fehler beim Erstellen von Request für Teil:', teil, error);
                }
            }
            ...
            try {
                await batch.commit();
                console.log(`✅ ${successCount} Material-Requests erfolgreich erstellt`);
                ...
                console.log(`✅ ${calendarEntries.length} Kalender-Einträge erstellt`);

            } catch (error) {
                console.error('❌ Fehler beim Speichern der Material-Requests:', error);
                return 0;
            }
        ...
        async function autoConvertErsatzteileToBestellungen(ersatzteile, kennzeichen, fahrzeugId) {
            if (!ersatzteile || ersatzteile.length === 0) {
                console.log('ℹ️ Keine Ersatzteile für Auto-Konvertierung');
                return 0;
            }
            ...
            console.log(`🔄 AUTO-PIPELINE: Konvertiere ${ersatzteile.length} Ersatzteile zu Bestellungen...`);

            const userName = localStorage.getItem('userName') || 'Admin';
            // ✅ SECURITY FIX: No 'mosbach' fallback - werkstattId must be validated at login
            if (!window.werkstattId) {
                console.error('❌ werkstattId nicht gesetzt - Bestellungen werden nicht erstellt');
                return 0;
            }
            const werkstattId = window.werkstattId;
            const batch = window.db.batch();
            ...
            for (const teil of ersatzteile) {
                ...
                if (!teil.etn || teil.etn.trim() === '') {
                    console.log(`   ⏭️ Überspringe Teil ohne ETN: ${teil.benennung}`);
                    bestellungenSkipped++;
                    continue;
                }
                ...
                try {
                    ...
                    const bestellungData = {
                        ...
                        kennzeichen: kennzeichen,
                        source: 'pdf-auto',  // Markierung für automatische Erstellung
                        sourceRequestId: null,  // Keine Material-Request (direkt aus PDF)

                        // ============================================
                        // NEUE FELDER (2025-11-11): Erweiterte Bestellverwaltung
                        // ============================================
                        preisGeschaetzt: parseFloat(teil.einzelpreis) || 0,    // Geschätzter Preis (für spätere Varianz-Berechnung)
                        preisTatsaechlich: null,                                // Tatsächlicher Preis (wird später editiert)
                    ...
                    bestellungenCreated++;
                    console.log(`   ✅ Bestellung erstellt: ${teil.etn} - ${teil.benennung}`);

                } catch (error) {
                    console.error(`   ❌ Fehler beim Erstellen von Bestellung für Teil:`, teil, error);
                }
            }
            ...
            if (bestellungenCreated > 0) {
                try {
                    await batch.commit();
                    console.log(`✅ AUTO-PIPELINE: ${bestellungenCreated} Bestellungen automatisch erstellt`);
                    if (bestellungenSkipped > 0) {
                        console.log(`ℹ️ AUTO-PIPELINE: ${bestellungenSkipped} Teile übersprungen (kein ETN)`);
                    }
                } catch (error) {
                    console.error('❌ Fehler beim Speichern der Auto-Bestellungen:', error);
                    return 0;
                }
            } else {
                console.log('ℹ️ AUTO-PIPELINE: Keine Bestellungen erstellt (alle Teile ohne ETN)');
            }
        ...
        /**
         * Handler für Text-Parsing Button
         */
        function handleTextParsing() {
            ...
            if (!text) {
                showToast('❌ Bitte Text einfügen', 'error');
                return;
            }

            try {
                console.log('📋 Parsing DAT-Text...');
                ...
                if (pdfData && pdfData.ersatzteile && pdfData.ersatzteile.length > 0) {
                    const confirmed = confirm(
                        `⚠️ Es wurden bereits ${pdfData.ersatzteile.length} Ersatzteile aus PDF extrahiert.\n\n` +
                        `Möchten Sie diese mit der Text-Extraktion ERSETZEN?\n\n` +
                        `💡 TIPP: Klicken Sie "Abbrechen" - Wir können beide Methoden KOMBINIEREN für höhere Zuverlässigkeit (97-98%)!`
                    );

                    if (!confirmed) {
                        // User wants to MERGE → Trigger hybrid merge!
                        console.log('🔄 User wählte MERGE → Kombiniere PDF + Text');
                        ...
                        statusSpan.textContent = `✅ ${teileCount} Ersatzteil${teileCount > 1 ? 'e' : ''} kombiniert (PDF + Text)`;
                        clearBtn.style.display = 'inline-block';

                        showToast(`✅ Kombiniert: ${teileCount} Ersatzteile (PDF + Text)`, 'success');
                        return;
                    } else {
                        // User confirmed REPLACE
                        console.log('⚠️ User wählte REPLACE → PDF-Daten werden überschrieben');
                    }
                }
                ...
                statusSpan.textContent = `✅ ${teileCount} Ersatzteil${teileCount > 1 ? 'e' : ''} gefunden`;
                clearBtn.style.display = 'inline-block';

                showToast(`✅ ${teileCount} Ersatzteile extrahiert`, 'success');

            } catch (error) {
                console.error('❌ Fehler beim Text-Parsing:', error);
                showToast('❌ Fehler beim Auslesen des Textes', 'error');
            }
        }
        ...
        function parseDatText(text) {
            ...
            // Optional: Arbeitslohn + Lackierung (später implementieren)
        ...
        /**
         * Normalisiert DAT-Text für zuverlässiges Parsing
         * Fixes:
        ...
        function normalizeDatText(text) {
            console.log('🔧 Normalizing DAT-Text...');
            ...
            console.log('✅ Text normalized:', normalizedLines.length, 'lines');
        ...
        /**
         * Lädt Ersatzteile in die editierbare Tabelle
         * @param {Array} ersatzteile - Array von Ersatzteil-Objekten (aus PDF oder manuell)
         */
        function renderErsatzteilePreview(ersatzteile) {
            if (ersatzteile && ersatzteile.length > 0) {
                // PDF-Daten → in globales Array laden
                ersatzteile.forEach(teil => addErsatzteilRow(teil));
                console.log(`✅ ${ersatzteile.length} Ersatzteile in Tabelle geladen (PDF-Import)`);
            }
        }

        /**
         * Fügt eine neue Ersatzteil-Zeile hinzu (leer oder mit Daten)
        ...
        /**
         * Aktualisiert die Kostenaufschlüsselung (sync mit allen 3 Tabellen)
         */
        function updateKostenaufschluesselung() {
            ...
            if (summeBrutto) summeBrutto.textContent = bruttoSumme.toFixed(2) + ' €';
            ...
            if (vereinbarterPreis && !manualOverride) {
                ...
                console.log(`✅ Vereinbarter Preis auto-updated: ${bruttoSumme.toFixed(2)} €`);
            } else if (manualOverride) {
                console.log(`⏭️ Vereinbarter Preis NICHT überschrieben (Manual Override aktiv)`);
            }
        }
        ...
        /**
         * Rendert Arbeitslohn-Preview (Labor Costs) - Lädt Daten in editierbare Tabelle
        ...
        function renderArbeitslohnPreview(arbeitslohn) {
            if (arbeitslohn && arbeitslohn.length > 0) {
                ...
                console.log(`✅ ${arbeitslohn.length} Arbeitslohn-Einträge in Tabelle geladen`);
            }
        }
        ...
        function addArbeitslohnRow(data = {}) {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 8px; text-align: center;">
                    <button type="button" onclick="deleteArbeitslohnRow(${rowIndex})" style="background: #f44336; color: white; border: none; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-size: 14px;">🗑️</button>
                </td>
            `;
        ...
        function updateArbeitslohnSumme() {
            ...
            document.getElementById('arbeitslohnTotal').textContent = summe.toFixed(2) + ' €';
        ...
        /**
         * Rendert Lackierung-Preview (Painting Work) - Lädt Daten in editierbare Tabelle
        ...
        function renderLackierungPreview(lackierung) {
            if (lackierung && lackierung.length > 0) {
                ...
                console.log(`✅ ${lackierung.length} Lackierung-Einträge in Tabelle geladen`);
            }
        }
        ...
        function addLackierungRow(data = {}) {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 10px; text-align: center;">
                    <button type="button" onclick="deleteLackierungRow(${rowIndex})" style="background: #f44336; color: white; border: none; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-size: 14px;">🗑️</button>
                </td>
            `;
        ...
        function updateLackierungSumme() {
            ...
            document.getElementById('lackierungTotal').textContent = summe.toFixed(2) + ' €';
        ...
        /**
         * Rendert Materialien-Preview - Lädt Daten in editierbare Tabelle
        ...
        function renderMaterialienPreview(materialien) {
            if (!materialien || materialien.length === 0) {
                console.warn('⚠️ PDF enthält keine Materialien-Daten. Materialien-Tabelle bleibt leer.');
                console.warn('   💡 Hinweis: Materialien können manuell über "+ Material hinzufügen" Button eingegeben werden.');
                return;
            }

            materialien.forEach(item => addMaterialienRow(item));
            console.log(`✅ ${materialien.length} Materialien-Einträge in Tabelle geladen`);
        }
            ...
            row.innerHTML = `
                ...
                <td style="padding: 10px;">
                    <select onchange="updateMaterialien(${rowIndex}, 'einheit', this.value)" style="width: 100%; border: 1px solid #ddd; border-radius: 4px; padding: 4px;">
                        ...
                        <option value="Stück" ${item.einheit === 'Stück' ? 'selected' : ''}>Stück</option>
                ...
                <td style="padding: 10px; text-align: center;">
                    <button type="button" onclick="deleteMaterialienRow(${rowIndex})" style="background: #f44336; color: white; border: none; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-size: 14px;">🗑️</button>
                </td>
            `;
        ...
        function updateMaterialienSumme() {
            ...
            document.getElementById('materialienTotal').textContent = summe.toFixed(2) + ' €';
        ...
        function updateGesamtsumme(pdfData) {
            ...
            if (nettoSumme > 0) {
                ...
                document.getElementById('summeErsatzteile').textContent = ersatzteileSumme.toFixed(2) + ' €';
                document.getElementById('summeArbeitslohn').textContent = arbeitslohnSumme.toFixed(2) + ' €';
                document.getElementById('summeLackierung').textContent = lackierungSumme.toFixed(2) + ' €';
                const summeMaterialienElement = document.getElementById('summeMaterialien');
                if (summeMaterialienElement) {
                    summeMaterialienElement.textContent = materialienSumme.toFixed(2) + ' €';
                }
                document.getElementById('gesamtpreis').textContent = nettoSumme.toFixed(2) + ' €'; // Netto in Preview

                gesamtsummeDiv.style.display = 'block';

                // ═══════════════════════════════════════════════════════════
                // NEU: AUTO-FILL KOSTENAUFSCHLÜSSELUNG (für Buchhaltung)
                // ═══════════════════════════════════════════════════════════
                ...
                document.getElementById('summeNetto').value = nettoSumme.toFixed(2) + ' €';

                // MwSt-Berechnung
                document.getElementById('mwstBetrag').value = mwstBetrag.toFixed(2);
                document.getElementById('summeBrutto').textContent = bruttoSumme.toFixed(2) + ' €';
                ...
                if (vereinbarterPreisInput) {
                    ...
                    console.log(`💰 Vereinbarter Preis auto-filled: ${bruttoSumme.toFixed(2)} € (Netto: ${nettoSumme.toFixed(2)} € + MwSt ${mwstSatz}%: ${mwstBetrag.toFixed(2)} €)`);
                }
        ...
        // ═══════════════════════════════════════════════════════════
        // EVENT HANDLER: MwSt-Satz Änderung + Manual Edit
        // ═══════════════════════════════════════════════════════════

        // MwSt-Dropdown: Neu-Berechnung bei Satz-Änderung
        document.getElementById('mwstSatz')?.addEventListener('change', function() {
            const netto = parseFloat(document.getElementById('summeNetto')?.value.replace(' €', '')) || 0;
            ...
            // Nur überschreiben wenn nicht manuell overridden
            const manualOverride = document.getElementById('manualPriceOverride')?.checked;
            if (!manualOverride) {
                document.getElementById('vereinbarterPreis').value = brutto.toFixed(2);
            }

            console.log(`📊 MwSt-Satz geändert: ${mwstSatz}% → Brutto: ${brutto.toFixed(2)} € ${manualOverride ? '(Manual Override aktiv)' : ''}`);
        });
        ...
        document.getElementById('manualPriceOverride')?.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                console.log('✏️ Manual Price Override: AKTIVIERT - Preis kann jetzt manuell editiert werden');
                showToast('✏️ Preis kann jetzt manuell editiert werden', 'info', 3000);
            } else {
                ...
                console.log('🔄 Manual Price Override: DEAKTIVIERT - Preis wird automatisch berechnet');
                showToast('🔄 Preis wird automatisch aus Tabellen berechnet', 'success', 3000);
            }
        });

        // 🔧 ENTFERNT (2025-11-27): Ersatzfahrzeug-Adresse Toggle
        ...
        document.getElementById('btnEditKosten')?.addEventListener('click', function() {
            ...
            if (isReadonly) {
                ...
                this.innerHTML = '<svg data-feather="check" style="width: 16px; height: 16px;"></svg> Änderungen übernehmen';
                this.style.background = '#28a745';

                if (typeof feather !== 'undefined') feather.replace();

                // Show notification
                alert('✏️ Kostenaufschlüsselung kann jetzt bearbeitet werden.\n\nDie MwSt und Gesamtsumme werden automatisch neu berechnet.');
            } else {
                // Save and lock
                fields.forEach(id => {
                    const input = document.getElementById(id);
                ...
                this.innerHTML = '<svg data-feather="edit-3" style="width: 16px; height: 16px;"></svg> Werte manuell bearbeiten';
                ...
                document.getElementById('summeNetto').value = netto.toFixed(2) + ' €';
                document.getElementById('mwstBetrag').value = mwstBetrag.toFixed(2);
                document.getElementById('summeBrutto').textContent = brutto.toFixed(2) + ' €';
                document.getElementById('vereinbarterPreis').value = brutto.toFixed(2);

                console.log(`✅ Kostenaufschlüsselung manuell aktualisiert: Netto=${netto.toFixed(2)} → Brutto=${brutto.toFixed(2)}`);
                alert('✅ Kostenaufschlüsselung aktualisiert!\n\nNeue Summe Brutto: ' + brutto.toFixed(2) + ' €');
            }
        });
        ...
        function extractErsatzteileFromText(text, data) {
            if (window.DEBUG) console.log('🔍 Extracting Ersatzteile from Text...');
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                // Pattern 2: ETN ohne Prefix (eingerückt, Teil von REPARATURSATZ)
                ...
                if (match) {
                    ...
                    // Swiss number format: 1'532.10 → 1532.10
                    ...
                    if (etn && benennung && anzahl > 0 && einzelpreis > 0 && gesamtpreis > 0) {
                        ...
                        console.log(`✅ Ersatzteil ${data.ersatzteile.length} gefunden:`, {
                            etn,
                            benennung,
                            anzahl,
                            einzelpreis: einzelpreis.toFixed(2),
                        ...
                        if (line.match(/\d{10}|\d{4,8}[A-Z]\d{0,2}/)) {
                            console.warn(`⚠️ Pattern-Match fehlgeschlagen für Zeile ${index + 1}:`, line);
                        }
                    }
                }
            }
            ...
            if (data.ersatzteile.length === 0) {
                console.warn('⚠️ Keine Ersatzteile im Text gefunden');
            } else {
                console.log(`✅ Insgesamt ${data.ersatzteile.length} Ersatzteile aus Text extrahiert`);
            }
        }
        ...
        function getFormData() {
            ...
            // 🆕 MULTI-SERVICE: Collect additional services
            const primaryServiceTyp = document.getElementById('serviceTyp').value;
            const additionalServices = Array.from(document.querySelectorAll('.additional-service-checkbox:checked'))
                // 🆕 BUGFIX 2025-11-15 (FIX #2): Prevent duplicates
                .filter((checkbox, index, self) => {
                    const serviceTyp = checkbox.value;

                    // Check 1: Nicht den Haupt-Service hinzufügen
                    if (serviceTyp === primaryServiceTyp) {
                        console.warn(`⚠️ [DUPLIKAT-CHECK] Service "${serviceTyp}" ist bereits Haupt-Service → wird übersprungen`);
                        return false;
                    }
                    ...
                    if (firstIndex !== index) {
                        console.warn(`⚠️ [DUPLIKAT-CHECK] Service "${serviceTyp}" bereits in additionalServices → wird übersprungen`);
                        return false;
                    }
                ...
                .map(checkbox => ({
                    serviceTyp: checkbox.value,
                    serviceDetails: getServiceDetails(checkbox.value)  // 🔧 FIX (2025-11-13): Renamed from serviceData to serviceDetails for consistency
                }));
            ...
            return {
                ...
                // 🆕 MULTI-SERVICE: Additional Services Array (Option C)
                ...
                pdfUploadedAt: null,
                status: 'angenommen',
                // 📦 ANLIEFERUNG (2025-11-27): Wenn Fahrzeug abgeholt werden muss ODER Ersatzfahrzeug gewünscht, startet es in "anlieferung"
                // 🔧 FIX (2025-11-27): 'angenommen' → 'terminiert' für Konsistenz mit Partner-App
                // 🆕 FIX (2025-11-29): Auch ersatzfahrzeugGewuenscht prüfen (wie meine-anfragen.html)
                prozessStatus: (document.querySelector('input[name="fahrzeugAbholung"]:checked')?.value === 'ja' || document.getElementById('ersatzfahrzeugGewuenscht')?.checked) ? 'anlieferung' : 'terminiert',
                // ✅ BUG #9 FIX (2025-11-26): werkstattId im Dokument speichern
                ...
                // 📦 ANLIEFERUNG (2025-11-27): prozessTimestamps berücksichtigt Anlieferung
                // 🔧 FIX (2025-11-27): 'angenommen' → 'terminiert' für Konsistenz mit Partner-App
                // 🆕 FIX (2025-11-29): Auch ersatzfahrzeugGewuenscht prüfen (wie meine-anfragen.html)
                prozessTimestamps: (() => {
                    const isAbholung = document.querySelector('input[name="fahrzeugAbholung"]:checked')?.value === 'ja';
                    const isErsatzfahrzeug = document.getElementById('ersatzfahrzeugGewuenscht')?.checked;
                    const initialStatus = (isAbholung || isErsatzfahrzeug) ? 'anlieferung' : 'terminiert';
                    return { [initialStatus]: timestamp };
                })(),
                // 🆕 NEW (2025-11-13): Multi-Service Status-Tracking
                // Ermöglicht paralleles Status-Tracking für Primary + Additional Services
                serviceStatuses: (() => {
                    const statuses = {};
                    const primaryService = document.getElementById('serviceTyp').value;
                    // 🆕 FIX (Bug 43): Besseres User-Tracking aus sessionStorage
                    ...
                    // 📦 ANLIEFERUNG (2025-11-27): Initial Status basiert auf Abholung ODER Ersatzfahrzeug
                    // 🔧 FIX (2025-11-27): 'angenommen' → 'terminiert' für Konsistenz mit Partner-App
                    // 🆕 FIX (2025-11-29): Auch ersatzfahrzeugGewuenscht prüfen (wie meine-anfragen.html)
                    const isAbholung = document.querySelector('input[name="fahrzeugAbholung"]:checked')?.value === 'ja';
                    const isErsatzfahrzeug = document.getElementById('ersatzfahrzeugGewuenscht')?.checked;
                    ...
                    // Primary Service: Status basiert auf Abholung
                    ...
                    // Additional Services: Status "neu" (wartend auf Primary Service)
                        ...
                        additionalServices.forEach(service => {
                            statuses[service.serviceTyp] = {
                                ...
                                statusHistory: [{
                                    ...
                                    timestamp: timestamp,
                                    user: currentUser,
                                    note: 'Wartet auf Primary Service'
                                }]
                            };
                        });
                    }
                ...
                manualPriceOverride: document.getElementById('manualPriceOverride')?.checked || false,
                // NEU: Kostenaufschlüsselung für Buchhaltung/Steuerberater
                ...
                // 🆕 FIX (2025-11-29): Ersatzfahrzeug-Felder in getFormData() hinzufügen
                // PROBLEM: Diese Felder wurden in prozessStatus-Berechnung verwendet, aber nicht gespeichert
                // LÖSUNG: Alle Ersatzfahrzeug-relevanten Felder explizit hinzufügen
                ersatzfahrzeugGewuenscht: document.getElementById('ersatzfahrzeugGewuenscht')?.checked || false,
        ...
        // 🖼️ PHOTO URL→BASE64 CONVERTER (FIX: Nov 20, 2025 - Pattern 35: PDF Photo Bug)
        // Converts Firebase Storage URLs to Base64 for jsPDF embedding
        async function convertPhotoUrlsToBase64(photoUrls) {
            if (!photoUrls || photoUrls.length === 0) return [];

            console.log(`🖼️ Konvertiere ${photoUrls.length} Fotos zu Base64 für PDF...`);
            const base64Photos = [];
            ...
            for (let i = 0; i < photoUrls.length; i++) {
                ...
                try {
                    ...
                    console.log(`   ✅ Foto ${i+1}/${photoUrls.length} konvertiert`);

                } catch (error) {
                    console.error(`   ❌ Foto ${i+1} konnte nicht geladen werden:`, error);
                    // Continue with remaining photos (graceful degradation)
                }
            }

            console.log(`✅ ${base64Photos.length}/${photoUrls.length} Fotos erfolgreich konvertiert`);
            return base64Photos;
        }
        ...
        async function createPDF(data) {
            ...
            // 🆕 PROFESSIONAL HEADER WITH BRANDING (2025-11-13)
            ...
            // 🎨 EXCEL-STYLE HEADER (FIX: Nov 20, 2025 - Pattern 36: Header abgeschnitten)
            // Kürzerer Text + kleinere Schrift für bessere Lesbarkeit
            doc.setTextColor(255, 255, 255);
            doc.setFontSize(18);  // Kleiner: 22 → 18
            doc.setFont(undefined, 'bold');
            doc.text('Annahmeprotokoll', logoLoaded ? 115 : 105, 18, { align: logoLoaded ? 'left' : 'center' });

            doc.setFontSize(10);  // Kleiner: 12 → 10
            ...
            // 🎨 EXCEL-STYLE TABELLEN (FIX: Nov 20, 2025 - Pattern 36)
            ...
            // Y-Position für nächsten Inhalt
            let y = doc.lastAutoTable.finalY + 10;

            // 🎨 TABELLE 3: SERVICES (Excel-Style - Pattern 36)
            ...
            // ┌─────────────────────────────────────────────┐
            // │ TABELLE 3: SERVICES (Excel-Style)           │
            // └─────────────────────────────────────────────┘
            doc.autoTable({
                startY: doc.lastAutoTable.finalY + 5,
            ...
            // Y-Position für Service-Details (NICHT ÄNDERN - Bug #2 Fix!)
            y = doc.lastAutoTable.finalY + 10;
            doc.setTextColor(0, 0, 0);
            doc.setFontSize(11);

            // 🆕 MULTI-SERVICE: Render Additional Services Details (NICHT ÄNDERN!)
            if (data.additionalServices && Array.isArray(data.additionalServices)
                && data.additionalServices.length > 0) {
                doc.setFont(undefined, 'normal');
                y += 6;
                ...
                data.additionalServices.forEach((additionalService) => {
                    ...
                    // 🆕 PAGE BREAK CHECK: Prevent header cutoff with many Additional Services
                    if (y > 210) {
                        doc.addPage();
                        y = 20;
                    }
                        ...
                        switch(addServiceTyp) {
                            ...
                            case 'tuev':
                                ...
                                if (addServiceData.faelligkeit) {
                                    doc.setFont(undefined, 'bold');
                                    doc.text('Fälligkeit:', 30, y);
                                    doc.setFont(undefined, 'normal');
                                    // faelligkeit ist String-Format "YYYY-MM", daher + '-01' für vollständiges Datum
                            ...
                            case 'folierung':
                                ...
                                if (addServiceData.folierungMaterial) {
                                    doc.setFont(undefined, 'bold');
                                    doc.text('Materialqualität:', 30, y);
                                    doc.setFont(undefined, 'normal');
                                ...
                                if (addServiceData.folierungInfo) {
                                    doc.setFont(undefined, 'bold');
                                    doc.text('Zusätzliche Informationen:', 30, y);
                                    doc.setFont(undefined, 'normal');
                            ...
                            case 'steinschutz':
                                ...
                                if (addServiceData.steinschutzBereiche) {
                                    doc.setFont(undefined, 'bold');
                                    doc.text('Zu schützende Bereiche:', 30, y);
                                    doc.setFont(undefined, 'normal');
                            ...
                            case 'werbebeklebung':
                                ...
                                if (addServiceData.werbebeklebungKomplexitaet) {
                                    doc.setFont(undefined, 'bold');
                                    doc.text('Design-Komplexität:', 30, y);
                                    doc.setFont(undefined, 'normal');
                                    const komplexLabels = {
                                        'einfach': 'Einfach',
                                        'mittel': 'Mittel',
                            ...
                            case 'lackier':
                                // FIX (2025-11-12): Verwende tatsächliche HTML-Feldnamen (farbname, farbnummer, etc.)
            ...
            if (data.fahrzeugAbholung === 'ja') {
                y += 12;
                doc.setFillColor(255, 152, 0); // Orange für Abholung
            ...
            // SERVICE-DETAILS (für service-spezifische Felder)
            if (data.serviceTyp && data.serviceTyp !== 'lackierung' && data.serviceDetails) {
                y += 12;
                doc.setFillColor(156, 39, 176); // Lila für Service-Details
                doc.roundedRect(15, y - 5, 180, 10, 3, 3, 'F');
                doc.setTextColor(255, 255, 255);
                doc.setFont(undefined, 'bold');
                doc.setFontSize(12);
                ...
                switch(data.serviceTyp) {
                    case 'reifen':
                        ...
                        if (data.serviceDetails.ersatzfahrzeugGewuenscht) {
                            ...
                            doc.setFont(undefined, 'normal');
                            doc.text(data.serviceDetails.ersatzfahrzeugGewuenscht ? 'Ja, gewünscht' : 'Nein', 60, y);
                            y += 7;
                        }
                    ...
                    case 'glas':
                        ...
                        if (data.serviceDetails.lieferoption) {
                            ...
                            const lieferoptionLabels = {
                                ...
                                'abholung': 'Abholservice gewünscht'
                            };
                    ...
                    case 'folierung':
                        ...
                        if (data.serviceDetails.folierungMaterial) {
                            doc.setFont(undefined, 'bold');
                            doc.text('Materialqualität:', 20, y);
                            doc.setFont(undefined, 'normal');
                            const materialLabels = {
                                'standard': 'Standard',
                                'premium': 'Premium',
                    ...
                    case 'steinschutz':
                        ...
                        if (data.serviceDetails.steinschutzBereiche) {
                            doc.setFont(undefined, 'bold');
                            doc.text('Zu schützende Bereiche:', 20, y);
                            doc.setFont(undefined, 'normal');
                    ...
                    case 'werbebeklebung':
                        ...
                        if (data.serviceDetails.werbebeklebungKomplexitaet) {
                            doc.setFont(undefined, 'bold');
                            doc.text('Design-Komplexität:', 20, y);
                            doc.setFont(undefined, 'normal');
                            const komplexLabels = {
                                'einfach': 'Einfach',
                                'mittel': 'Mittel',
            ...
            // 🆕 LACKIER-INFO (Moved from after Fahrzeugdaten - now appears with Service Details)
            ...
            if (data.serviceTyp === 'lackier' || data.serviceTyp === 'alle' || hasPaintData) {
                // 🆕 Page-Break-Check VOR LACKIER-INFO (Fix: Sektion wurde abgeschnitten!)
                // Der Lackier-Info Block braucht ca. 55mm Höhe (Header + 4 Felder + FARBNUMMER-Box)
                if (y > 200) {
                    doc.addPage();
                    y = 20;
                }
            ...
            // 💰 PREISE-BERECHTIGUNG: Nur anzeigen wenn Berechtigung vorhanden
            ...
            if (canShowPrices && data.kva && data.kva.varianten && data.gewaehlteVariante) {
                ...
                // Header
                doc.setFillColor(33, 150, 243);  // Blau für KVA
                ...
                // 🆕 MULTI-SERVICE BREAKDOWN RENDERING
                ...
                if (data.kva.isMultiService && data.kva.breakdown) {
                    ...
                    console.log('📦 [PDF] Rendere Multi-Service Breakdown');

                    const breakdown = data.kva.breakdown;
                    const serviceLabels = data.kva.serviceLabels || {};
                    const serviceNames = {
                        'lackierung': '🎨 Lackierung',  // 🔧 FIX: lackierung (konsistent mit SERVICE_TEMPLATES)
                        'reifen': '🛞 Reifen-Service',
                        'mechanik': '🔧 Mechanik-Service',
                        'pflege': '✨ Fahrzeug-Pflege',
                        'tuev': '📋 TÜV/AU',
                    ...
                    // Für jeden Service eine Section
                    Object.keys(breakdown).forEach((serviceTyp, index) => {
                        ...
                        service.fields.forEach(fieldName => {
                            ...
                            doc.text(beschreibung, 20, y);
                            doc.setFont(undefined, 'bold');
                            doc.text(value.toFixed(2) + ' €', 170, y);
                            doc.setFont(undefined, 'normal');
                        ...
                        doc.text(service.gesamt.toFixed(2) + ' €', 170, y);
                        doc.setFont(undefined, 'normal');
                        doc.setFontSize(9);
                        y += 8;
                    });
            ...
            y += 8;  // Optimiert: 12 → 8 (4px weniger für bessere Seitenauslastung)
            doc.setFont(undefined, 'bold');
            ...
            if (data.notizen) {
                ...
                y += notizen.length * 5 + 5;  // Optimiert: 10 → 5 (weniger Padding nach Notizen)
            } else {
            ...
            if (photosForPDF.length > 0) {
                if (y > 245) {  // Optimiert: 220 → 245 (mehr Content auf Seite 1)
                    doc.addPage();
                    y = 20;
                }
            ...
            if (data.qrCodeUrl) {
                try {
                    console.log('🔲 Generiere QR-Code für PDF (neben Unterschrift)...');
                    ...
                    if (data.qrCodeExpiresAt) {
                        ...
                        doc.text('Gültig bis: ' + expiryDate, qrX + (qrSize / 2), qrY + qrSize + 3, { align: 'center' });
                    }

                    // PASSWORT SECTION (nur für NEU-KUNDEN)
                    if (data.isNewPartner && data.partnerPassword) {
                        ...
                        doc.text('(Beim 1. Login ändern)', qrX + (qrSize / 2), pwY + 11, { align: 'center' });

                        // 🆕 FIX #4: Passwort nicht mehr loggen (Security) - 2025-11-15
                        console.log('   ✅ Passwort im PDF angezeigt (Länge:', data.partnerPassword?.length || 0, 'Zeichen)');
                    } else {
                        console.log('   ✅ BESTANDSKUNDE → Kein Passwort im PDF');
                    }
                    ...
                    console.log('✅ QR-Code erfolgreich neben Unterschrift platziert');

                } catch (qrError) {
                    console.error('❌ QR-Code Generierung fehlgeschlagen:', qrError);
                }
            } else {
                console.log('⏭️ Kein QR-Code URL → Übersprungen');
            }
        ...
        async function erstelleEntwurfPDF(entwurfData, qrCodeUrl) {
            console.log('📄 Erstelle Entwurf-PDF mit Excel-Style Layout...');
            ...
            doc.setFont(undefined, 'bold');
            doc.text('Fahrzeug-Annahme', 105, 18, { align: 'center' });

            doc.setFontSize(14);
            doc.text('⚠️ ENTWURF', 105, 28, { align: 'center' });

            doc.setFontSize(10);
            doc.setFont(undefined, 'normal');
            doc.text(werkstattName, 105, 36, { align: 'center' });
            ...
            // 🎨 EXCEL-STYLE FAHRZEUGDATEN TABLE
            ...
            // 🎨 EXCEL-STYLE SERVICES TABLE (Multi-Service Support)
            y = doc.lastAutoTable.finalY + 5;
            ...
            const serviceLabels = {
                ...
                'pflege': 'FAHRZEUG-PFLEGE', 'tuev': 'TÜV/AU-PRÜFUNG', 'alle': 'ALLE SERVICES',
            ...
            if (qrCodeUrl) {
                try {
                    console.log('🔲 Generiere QR-Code für Partner-Login...');
                    ...
                    // Light gray background box
                    doc.setFillColor(245, 245, 245);
                    doc.roundedRect(qrX - (qrSize / 2) - 5, qrY - 10, qrSize + 10, qrSize + 30, 3, 3, 'F');

                    // "PARTNER-PORTAL" Label above QR
                    doc.setFontSize(14);
                    doc.setFont(undefined, 'bold');
                    doc.setTextColor(0, 51, 102);
                    doc.text('[PARTNER] PARTNER-PORTAL ZUGANG', qrX, qrY - 5, { align: 'center' });
                    ...
                    doc.setFont(undefined, 'normal');
                    doc.setTextColor(80, 80, 80);
                    doc.text('QR-Code scannen für Partner-Login', qrX, qrY + qrSize + 8, { align: 'center' });

                    // URL below (kleinere Schrift)
                    doc.setFontSize(7);
                    doc.text(qrCodeUrl, qrX, qrY + qrSize + 13, { align: 'center' });

                    console.log('✅ QR-Code erfolgreich im Entwurf-PDF platziert');
            ...
            // 🔑 PASSWORD SECTION (for new partners only - FIX: Nov 21, 2025)
            if (entwurfData.isNewPartner && entwurfData.partnerPassword) {
                ...
                doc.setFontSize(8);
                doc.setTextColor(100, 100, 100);
                doc.text('(!) Passwort muss beim ersten Login geändert werden', pwX, y + 14, { align: 'center' });

                y += 25;  // Update y for footer

                console.log('✅ Passwort erfolgreich im Entwurf-PDF platziert');
            }
            ...
            console.log('✅ Entwurf-PDF erfolgreich erstellt:', filename);
            return doc.output('blob');
        }
        ...
        async function createArbeitsauftrag() {
            ...
            // Farbnummer nur bei Lackierung & Alle Services prüfen
            if ((serviceTyp === 'lackier' || serviceTyp === 'alle') && !document.getElementById('farbnummer').value) {
                showToast('Bitte Farbnummer für Lackierung/Arbeitsauftrag eingeben!', 'error');
                return;
            }
...
// ============================================
// THEME TOGGLE + UX ANIMATIONS (ausgelagert aus inline script)
// ============================================
        // ============================================
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (pdfInput) {
                ...
                console.log('✅ PDF-Upload Event Listener registered');
            }
        });
    ...
    <!-- ============================================
         🎨 UX REDESIGN PHASE 1-5 JavaScript
    ...
    <script>
        ...
        function init3DTiltEffect() {
            ...
            console.log('✅ 3D Tilt Effect initialized for', checkboxCards.length, 'checkbox cards');
        }
        ...
        function initRippleEffect() {
            ...
            console.log('✅ Ripple Effect initialized for', rippleElements.length, 'buttons');
        }
        ...
        function initScrollAnimations() {
            if (!('IntersectionObserver' in window)) {
                console.log('ℹ️ Intersection Observer not supported');
                return;
            }
            ...
            console.log('✅ Scroll Animations initialized for', animatableElements.length, 'sections');
        }
        ...
        function initGSAPAnimations() {
            if (typeof gsap === 'undefined') {
                console.warn('⚠️ GSAP not loaded - entry animations disabled');
                return;
            }
            ...
            console.log('✅ GSAP Entry Animations initialized');
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('🎨 UX Redesign Phases 1-5 initialized for annahme.html');
        });
